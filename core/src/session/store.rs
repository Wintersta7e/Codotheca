...
/// nothing after it. `max(…, 0)` because a wall clock can move backwards, and the guard on
/// `closed_by IS NULL` is what stops a late activity batch reopening a segment already closed.
