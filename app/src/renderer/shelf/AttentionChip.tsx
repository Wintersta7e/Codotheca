...
export interface AttentionChipProps {
  ...
  /**
   * §8.8's broken outline, `inset 0 0 0 1px --fail-hot`. Outline only: the ground and the type
   * are unchanged, because what is broken is the query behind the chip, not the control. The
   * shelf half declared `.cdt-attention-chip--broken` and applied it before this component was
   * extracted; extracting the box without this prop dropped the state silently, which is why
   * it is a named prop rather than a caller-supplied class.
   */
...
export function AttentionChip(props: AttentionChipProps): ReactElement {
  ...
  return (
    <button
      type="button"
      className={`cdt-attention-chip${props.broken === true ? ' cdt-attention-chip--broken' : ''}`}
