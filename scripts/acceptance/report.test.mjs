...
test('countByPhase splits the register without a second file', () => {
  ...
  assert.equal(counts[2].criteria, 68);
  assert.equal(counts[2].checks, 136);
  assert.equal(
...
test('the line a successful run prints states what it validated, per phase', () => {
  ...
  assert.equal(
    ...
    '138 criteria / 307 checks validated — phase 1 70/171, phase 2 68/136',
  );
  assert.equal(
...
test('the run report splits its check count by phase', () => {
  ...
  assert.match(rendered, /Phase 2: 136 checks/u);
});
