...
test('countByPhase splits the register without a second file', () => {
  ...
  assert.equal(counts[2].criteria, 66);
  assert.equal(counts[2].checks, 133);
  assert.equal(
...
test('the line a successful run prints states what it validated, per phase', () => {
  ...
  assert.equal(
    ...
    '136 criteria / 304 checks validated — phase 1 70/171, phase 2 66/133',
  );
  assert.equal(
...
test('the run report splits its check count by phase', () => {
  ...
  assert.match(rendered, /Phase 2: 133 checks/u);
});
